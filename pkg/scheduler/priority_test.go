...
func TestRescore(t *testing.T) {
	p := Priority{QCrit: 0.8}
	p.Rescore()
	if diff := cmp.Diff(0.8, p.Score); diff != "" {
		t.Errorf("Score mismatch (-want +got):\n%s", diff)
	}
}

func TestScoreWithSubstitutesCriticality(t *testing.T) {
	// Scoring a version swaps in that version's criticality while keeping the
	// package's other signals, so a busy old version can outrank a fresh one.
	p := Priority{QCrit: 0.2}
	if got, want := p.ScoreWith(0.9), 0.9; got != want {
		t.Errorf("ScoreWith(0.9) = %v, want %v", got, want)
	}
	if p.Score != 0 {
