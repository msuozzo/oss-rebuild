...
func rebuildPackage(ctx context.Context, req schema.RebuildPackageRequest, deps *RebuildPackageDeps) (*schema.Verdict, error) {
	...
	v := schema.Verdict{Target: t}
	if !deps.OverwriteAttestations {
		if exists, err := a.BundleExists(ctx, t); err != nil {
			return &v, api.AsStatus(codes.Internal, errors.Wrap(err, "checking existing bundle"))
		} else if exists {
			return &v, api.AsStatus(codes.AlreadyExists, errors.New("conflict with existing attestation bundle"))
		}
	}
...
func RebuildPackage(ctx context.Context, req schema.RebuildPackageRequest, deps *RebuildPackageDeps) (*schema.Verdict, error) {
	...
	if err != nil {
		return v, api.AsStatus(codes.Internal, errors.Wrap(err, "storing results in firestore"))
	}
	return v, nil
}
