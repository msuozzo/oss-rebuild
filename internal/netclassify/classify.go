...
// Maven
var (
	mavenCentralRegex = regexp.MustCompile(`^https?://(repo1\.maven\.org/maven2|central\.maven\.org/maven2|maven-index://repo1\.maven\.org)/(.+)/([^/]+)/([^/]+)/([^/]+)$`)
)
...
func ClassifyURL(rawURL string) (string, error) {
	if pat := matchOCIRegistry(rawURL); pat != nil {
		...
		return "", ErrSkipped
	} else if mavenCentralRegex.MatchString(rawURL) {
...
func classifyMavenURL(rawURL string) (string, error) {
	matches := mavenCentralRegex.FindStringSubmatch(rawURL)
	if len(matches) < 6 {
